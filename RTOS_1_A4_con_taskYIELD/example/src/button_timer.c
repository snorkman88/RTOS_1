...
static void vLed3TaskD(void *pvParameters) {
	...
	while(1){
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, LED3[0], LED3[1]);
		delay_con_for(500);
	}
...
static void vLed2TaskC(void *pvParameters) {
	...
	{
		taskYIELD();
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, LED2[0], LED2[1]);
		taskYIELD();
		delay_con_for(250);
	}
	vTaskSuspend(0);
...
static void vLed1TaskB(void *pvParameters) {
	...
	{
		taskYIELD();
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, LED1[0], LED1[1]);
		taskYIELD();
		delay_con_for(250);
	}
...
static void vLedATaskA(void *pvParameters) {
	...
	xTaskCreate(vLed1TaskB,
			...
			300,
			NULL,
			tskIDLE_PRIORITY+2,//antes era +3
			&handlerLed1);
	...
	for(i=0; i<4; i++)
	{
		Chip_GPIO_SetPinToggle(LPC_GPIO_PORT, LEDA[0], LEDA[1]);
		delay_con_for(500);
	}
